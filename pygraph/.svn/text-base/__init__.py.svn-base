...
class tkGraphView(object):
    ...
    def __init__(self, nodeEdgeAry, title, outfile='',root=None, graphtype='directed',startTk=False):
        ...
        # add a "save" button
        Tkinter.Button(self._root, text='Save',
                           background='#a0f0c0', foreground='black', 
                           command=self.writeImage).pack(side='top')
    ...
    def writeImage(self):
        if self.imagefile: 
            # 75 dpi
            self.canvas.postscript(file=self.imagefile+'.ps', width=self.xdim, height=self.ydim, pagewidth=self.xdim,pageheight=self.ydim)
